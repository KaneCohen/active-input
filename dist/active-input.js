	...
	(function (root, factory) {
	  ...
	  if (window.jQuery) {
	    ...
	    jQuery.fn.activeInput = function (options) {
	      ...
	      this.each(function () {
	        o.element = jQuery(this)[0];
	        ai = util.data(o.element, 'activeInput');
	        if (ai) {
	  ...
	  */
	  ...
	  var util = {
	    ...
	    data: function data(el, prop, value) {
	      var p = 'data';
	      var data = el[p] || {};
	    ...
	    parents: function parents(el, els) {
	      els || (els = []);
	      var p = el.parentNode;
	      if (p) {
	        els.push(p);
	    ...
	    isDisabled: function isDisabled(el) {
	      ...
	      this.parents(el).forEach(function (v) {
	    ...
	    findInputElements: function findInputElements(el) {
	      var self = this;
	    ...
	    serializeObject: function serializeObject(el) {
	      var self = this,
	          data = {};
	      var els = this.findInputElements(el);
	      els.forEach(function (input, key) {
	    ...
	    buildObject: function buildObject(input, pointer, keys) {
	      var l = keys.length,
	          item,
	          i,
	          k;
	  ...
	  ActiveInput.prototype = {
	    init: function init() {
	      ...
	      if (this.v.input.length === 0) {
	        throw new Error('ActiveInput.Error: activeInput must be bind to at least one input element.');
	      }
	      this.v.button = o.button;
	      if (this.o.button === null) {
	        var buttons = o.element.querySelectorAll('button');
	        if (buttons.length === 0) buttons = o.element.querySelectorAll('input[type="submit"]');
	        if (buttons.length === 0) {
	          throw new Error('ActiveInput.Error: activeInput container must have button or submit element available.');
	        }
	      ...
	      util.data(this.o.element, 'activeInput');
	    ...
	    validate: function validate(input, e) {
	      var self = this;
	    ...
	    trigger: function trigger(name) {
	      ...
	      if (this.v.events[name]) {
	        if (this.v.events[name].apply(this, args) === false) return false;
	      }
	      if (this[name]) {
	        if (this[name].apply(this, args) === false) return false;
	      }
	      return true;
	    }
	  };
